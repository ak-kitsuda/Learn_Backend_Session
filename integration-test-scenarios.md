...
- ✅ 成功メッセージ: 「ユーザー登録が完了しました！ログインページに移動します...」
- ✅ 2 秒後に自動的に `/login` ページに遷移
...
- ✅ Todo 作成フォームが表示（タイトル、説明、優先度選択）
- ✅ 統計情報エリアが表示（総数: 0, 完了: 0, 未完了: 0）
- ✅ 「📤 ログアウト」ボタンが表示
- ✅ 「現在 Todo はありません」メッセージが表示
...
1. 1 つ目の Todo（学習計画を立てる）の「✅ 完了」ボタンをクリック
...
**操作手順（続き）**: 2. 3 つ目の Todo（UI 改善案検討）の「🗑️ 削除」ボタンをクリック
...
**操作手順（続き）**: 3. 完了済みの Todo（学習計画を立てる）の「↩️ 戻す」ボタンをクリック
